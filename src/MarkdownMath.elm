module MarkdownMath exposing
  ( toHtml
  , Options, defaultOptions, toHtmlWith
  )

{-| A library for markdown parsing. This is just an Elm API built on top of the
[marked](https://github.com/chjj/marked)  and [katex](https://github.com/Khan/KaTeXproject)
which focuses on speed.

# Parsing Markdown and Math
@docs toHtml
...
import Native.MarkdownMath


{-| Turn a markdown/math string into an HTML element, using the `defaultOptions`.

    recipe : Html msg
    recipe =
       MarkdownMath.toHtml [class "recipe"] """
...
toHtml attrs string =
  Native.MarkdownMath.toHtml defaultOptions attrs string
...
`<script>` tags, you can use modified parsing options.
    ...
    toMarkdown userInput =
        MarkdownMath.toHtmlWith options [] userInput
-}
toHtmlWith : Options -> List (Attribute msg) -> String -> Html msg
toHtmlWith =
  Native.MarkdownMath.toHtml
